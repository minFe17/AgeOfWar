...
public class PlayerRangeUnit : MonoBehaviour
{
    ...
    public void PriestUnitScan()
    {
        if(!_isDie)
        {
            ...
            _scanEnemys = new Transform[length];
            for(int i=0; i<hit.Length; i++)
            {
                if(hit[i].transform != null)
                {
                    _scanEnemys[i] = hit[i].transform;
                    _isMove = false;
                    _animator.SetBool("isMove", false);
                    if(!_isAttack && !_isDie)
                    {   
    ...
    IEnumerator PriestAttackRoutine()
    {
        if(_scanEnemys != null)
        {
            ...
            for(int i=0; i<_scanEnemys.Length; i++)
            {
                bulletInstance = Instantiate(_bulletPrefab, _scanEnemys[i]);
            }
